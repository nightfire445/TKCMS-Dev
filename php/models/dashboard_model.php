...
class Model
{
    ...
    public function editVendor(){
        ...
        if( $menu_upload_result ){
            ...
            if($status_get_menu){
                $update_menu_query  = $this->dbconn->prepare("UPDATE `menu` SET `menu_url` = :menu_url WHERE `vendor_FK` = :id");
                $status = $update_menu_query->execute(array(':id' => $vendor_id, ':menu_url' => $menu_url));

            }
            else{
                $insert_menu = $this->dbconn->prepare("INSERT INTO `menu` (`menu_url`, `vendor_FK`) VALUES (:menu_url, :vendor_id )");
                $status = $insert_menu->execute(array(':vendor_id' => $vendor_id, ':menu_url' => $menu_url));
